...
ghostty-shaders is a personal terminal-theming tool. Its attack surface is
...
`ghostty-shaders weather off` or manually via `launchctl`.

**Outbound HTTPS to Open-Meteo.** The poller makes one HTTPS request per
5-minute poll to `https://api.open-meteo.com`. No authentication header or
...
`~/.config/ghostty-shaders/config.env`, those coordinates go only to
...
are written to `~/.config/ghostty-shaders/location.json`; all subsequent
...
- `~/Library/Caches/ghostty-shaders/` — rotated shader copies (the 10 most
...
- `~/.config/ghostty-shaders/` — `config.env`, `location.json`, and
  `active.conf` (the include Ghostty reads).
- `~/Library/Logs/ghostty-shaders-poll.log` — the poller's log,
...
compilation fails, `ghostty-shaders apply` detects the error in Ghostty's log
